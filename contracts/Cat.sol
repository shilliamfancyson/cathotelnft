...
contract Cat is ERC721Enumerable, Ownable {
  ...
  uint256 public catSupply = 0;
  uint256 public cnSupply = 0;
  uint256 public catsNapped = 0;
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    //uint256 seed = catSupply + cnSupply;
    ...
    for (uint256 i = 1; i <= _mintAmount; i++) {
        ...
        if (generateSeed(catSupply+cnSupply,10) != 0){
          ...
          //mint a catnapper
          address recipient = selectReceipient();
          _safeMint(recipient, cnSupply + 45001);
          cnSupply += 1;
        }
  ...
  function selectReceipient() public view returns (address){
      uint256 seed = generateSeed(catSupply+cnSupply,10);
      if (seed == 0)
      {
        address catNapper = hotel.randomCatNapper(seed);
        //catsNapped += 1;
        return catNapper;
      } else {
        return msg.sender;
